...
export function ResidentQrIdentityTab({ resident }: { resident: ResidentDetailResponse }) {
  ...
  return <div className="space-y-4">
    ...
    <section className="grid gap-4 lg:grid-cols-[320px_1fr]">
      ...
      <SectionCard title="Identity status"><div className="flex flex-wrap items-center gap-2"><StatusBadge label={identity?.status || "Not issued"} tone={identity?.status === "active" && resident.system.is_active ? "success" : "warning"} />{!resident.system.is_active ? <StatusBadge label="Inactive resident" tone="danger" /> : null}</div><dl className="mt-4 space-y-2 text-sm"><div><dt className="text-slate-500">Public identifier</dt><dd className="font-mono font-medium">{identity?.identifier || resident.qr_profile.code}</dd></div><div><dt className="text-slate-500">Issued</dt><dd>{identity ? <LocalDateTime value={identity.issued_at} /> : "Legacy identity not registered"}</dd></div><div><dt className="text-slate-500">Issued by</dt><dd>{identity?.issued_by || "System"}</dd></div></dl><div className="mt-5 flex flex-wrap gap-2">{actions.issue ? <PrimaryButton disabled={busy} onClick={() => void act("issue")}>Issue QR Identity</PrimaryButton> : null}{identity?.status === "active" ? <><SecondaryButton disabled={busy || !actions.reissue} onClick={() => void act("reissue")}>Reissue</SecondaryButton><SecondaryButton disabled={busy || !actions.revoke} onClick={() => void act("revoke")}>Revoke</SecondaryButton></> : null}<Link href={`/verify/resident/${encodeURIComponent(identity?.identifier || resident.qr_profile.code)}`} target="_blank" className="rounded-md border border-slate-300 px-3 py-2 text-sm font-medium hover:bg-slate-50">Public Verification</Link><Link href={`/residents/documents/barangay-id/sample/${resident.identity.id}/`} target="_blank" className="rounded-md border border-slate-300 px-3 py-2 text-sm font-medium hover:bg-slate-50">Print QR ID</Link></div>{identity?.status === "active" ? <label className="mt-4 block text-sm"><span className="mb-1 block font-medium">Required reason for reissue or revocation</span><textarea value={reason} onChange={(event) => setReason(event.target.value)} rows={3} className="w-full rounded-md border border-slate-300 px-3 py-2" /></label> : null}</SectionCard>
    </section>
